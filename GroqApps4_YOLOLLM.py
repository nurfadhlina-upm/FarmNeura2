...
from ultralytics import YOLO  # Powered by Ultralytics open-vocabulary framework
...
def load_yolo_world_model():
    # Utilizing YOLO-World to eliminate blind COCO classifications (chairs, ties, etc.)
    # It dynamically recalibrates its visual attention filters to seek green leaf objects
    model = YOLO("yolov8s-world.pt")
    model.set_classes(["leaf", "green leaf"])
    return model


yolo_model = load_yolo_world_model()
...
if images_to_process:
    ...
    for index, img_data in enumerate(images_to_process):
        ...
        with col1:
            st.markdown("### 🔍 YOLO-World Zero-Shot Output")
            
            # --- PHASE 1: HIGH-PRECISION COMPUTER VISION DETECTION ---
            # Set a slightly strict confidence threshold to screen out background weed foliage
            yolo_results = yolo_model(original_image, conf=0.20, verbose=False)
            yolo_boxes = yolo_results[0].boxes.xyxy.cpu().numpy()
            yolo_count = len(yolo_boxes)

            # Paint exact bounding rectangles directly over the localized coordinates
            ...
            # Output the sharp marked image matrix onto Column 1
            st.image(
                annotated_image,
        ...
        with col2:
            ...
            with st.spinner("Generating diagnostic assessment via LLM..."):
                try:
                    ...
                    # Render pathology matrix cleanly without coordinate table clutter
